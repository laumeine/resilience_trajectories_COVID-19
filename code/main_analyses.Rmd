...
```{r import and format data, include=F}
...
# fix some participant codes in pandemic data where German umlaut is not correctly displayed for some reason
data_pandemic$Code[data_pandemic$Code=="AN25R‹JU"] <- "AN25RÜJU"
data_pandemic$Code[data_pandemic$Code=="GA20K÷NA"] <- "GA20KÖNA"
data_pandemic$Code[data_pandemic$Code=="IL09WöLI"] <- "IL09WÜLI"
data_pandemic$Code[data_pandemic$Code=="OH02OFBR"] <- "OH2OFBR" # typo
data_pandemic$Code[data_pandemic$Code=="ON18RöIS"] <- "ON18RÜIS"
data_pandemic$Code[data_pandemic$Code=="ST03KôBI"] <- "ST03KÖBI"
data_pandemic$Code[data_pandemic$Code=="AN09WöJA"] <- "AN09WÜJA"
data_pandemic$Code[data_pandemic$Code=="ƒR04MAFR"] <- "ÄR04MAFR"
data_pandemic$Code[data_pandemic$Code=="LK11GôKA"] <- "LK11GÖKA"
data_pandemic$Code[data_pandemic$Code=="LK26RöAL"] <- "LK26RÜAL"
data_pandemic$Code[data_pandemic$Code=="SA29R‹DO"] <- "SA29RÜDO"
...
data_wide_pandemic <- data_wide_pandemic %>% 
  filter(! is.na(Cohort)) # 117/133 participants completed follow-up
```

```{r compute T4 resilient functioning score, include=F}
# compute resilient functioning score during pandemic (T4)

# indicators of mental health
mh_items <- data_wide_pandemic %>% select(BSI_P_GSI_INV, WHO_P)

# indicators of stress
stress_items <- data_wide_pandemic %>% select(MIMIS_P_freq, LES_P_Count)
...
summary(results$Stress_PCA) # stress component 1 explains 72% variance

# add resulting scores to data
data_wide_pandemic$RF_P <- unlist(results[1])
data_wide_pandemic$stressPCA_P <- unlist(results[2])
data_wide_pandemic$mhPCA_P <- unlist(results[3])

# check data
grep("BSI_P_GSI_INV", colnames(data_wide_pandemic)) # index BSI_GSI inverted: 6
grep("WHO_P", colnames(data_wide_pandemic)) # index WHO: 51

index_max_mh <- which.max(data_wide_pandemic$mhPCA_P) # id with highest mh component 1 score
data_wide_pandemic[index_max_mh, c(6,51)] # high WHO and BSI score, so high mh component score correctly indicates high mh
index_min_mh <- which.min(data_wide_pandemic$mhPCA_P) # id with lowest mh component 1 score
data_wide_pandemic[index_min_mh,c(6,51)] # low WHO and BSI score, so low mh component score correctly indicates low mh

grep("MIMIS_P_freq", colnames(data_wide_pandemic)) # index MIMIS Frequency: 49
grep("LES_P_Count", colnames(data_wide_pandemic)) # index LES Frequency: 47

index_max_stress <- which.max(data_wide_pandemic$stressPCA_P) # id with highest stress component 1 score
data_wide_pandemic[index_max_stress, c(49,47)] # high MIMIS and LES score
index_min_stress <- which.min(data_wide_pandemic$stressPCA_P) # id with lowest stress component 1 score
data_wide_pandemic[index_min_stress,c(49,47)] # low MIMIS and LES score
```
...
cohort4 <- (table(data_wide$Cohort)[2]/nrow(data_wide))*100 # % cohort 4 
```
...
slightly reduced sample for pandemic time point
...
cohort4P <- (table(data_wide_pandemic$Cohort)[2]/nrow(data_wide_pandemic))*100 # % cohort 4 
...
```
Data from *Np* = `r round(Np,2)` participants aged `r round(age.minP,2)`-`r round(age.maxP,2)` (`r round(femaleP,2)` % female, age: *M* = `r round(age.mP,2)`, *SD* = `r round(age.sdP,2)`) could be analyzed for the time point during the lockdown.
...
options(scipen=999) # don't show results in exponential notation
...
## Plot Trajectories with individual data
...
RF_class_cohort <- data_wide %>%
  group_by(class) %>%
  tally(mean(Cohort==4)*100) # % cohort 4

# Chi-square test
chisq.test(data_wide$Cohort, data_wide$class) # cohort and class membership are dependent!
```
...
Since resilient functioning score does not directly show how different classes score on stress and mental health, we output the average across time points for all measures that make up the score (BSI, WHO, MIMIS, LES).
...
RF_class_means_who <- summarySEwithin2(data_long, measurevar="WHO", betweenvar="class", withinvar="timepoint", idvar="Code", conf.interval=.95)

WHOtrajectories <- ggplot(RF_class_means_who, aes(x=timepoint, y=WHO, group=class, color=class)) +
  geom_line(size=1.5) +
  geom_point(aes(shape=class), size=4) +
...
```{r class descriptives - MIMIS}
MIMIS_freqs <- data_wide %>% select("MIMIS_T0_Frequency", "MIMIS_T1_Frequency", "MIMIS_T2_Frequency", "MIMIS_T3_Frequency")
data_wide$MIMIS_mean_Frequency <- rowMeans(MIMIS_freqs)

RF_class_means_MIMIS <- data_wide %>%
  group_by(class) %>%
  summarise_at(vars(MIMIS_mean_Frequency),
               list(mean = mean,
                    sd = sd))

# get outlier IDs
mimisOutlierIDs <- data_wide %>%
  identify_outliers(MIMIS_mean_Frequency)

# to be conservative and not exclude too much data, we will only consider extreme outliers
mimisOutlierIDs$Code[mimisOutlierIDs$is.extreme==T] # no extreme outliers

# check assumptions for one-way ANOVA
mimisModel <- lm(MIMIS_mean_Frequency ~ class, data_wide)

# homogeneity of variance
plot(mimisModel, 1) 
levene_test(mimisModel)

# normality of residuals
plot(mimisModel, 2)
shapiro.test(residuals(mimisModel)) # non-normal distribution

# use Kruskal-Wallis test instead of ANOVA
kruskal.test(MIMIS_mean_Frequency ~ class, data_wide) # p < .05

# plot trajectories of MIMIS freq
RF_class_means_mimis <- summarySEwithin2(data_long, measurevar="MIMIS_Frequency", betweenvar="class", withinvar="timepoint", idvar="Code", conf.interval=.95)

MIMIStrajectories <- ggplot(RF_class_means_mimis, aes(x=timepoint, y=MIMIS_Frequency, group=class, color=class)) +
  geom_line(size=1.5) +
  geom_point(aes(shape=class), size=4) +
...
# boxplot by class
optBoxPlot <- ggplot(data_wide, aes(class, LOT_Opt_T0, fill=class)) + 
  geom_violin() +  
...
optOutlierIDs <- data_wide %>%
  identify_outliers(LOT_Opt_T0)

# to be conservative and not exclude too much data, we will only consider extreme outliers
optOutlierIDs$Code[optOutlierIDs$is.extreme==T] # no extreme outliers

# check assumptions for one-way ANOVA
optModel  <- lm(LOT_Opt_T0 ~ class, data = data_wide)
...
# one-way ANOVA
opt.fit <- aov_ez("Code","LOT_Opt_T0", data_wide, between=c("class"))
nice(opt.fit, es="pes") # p < .001

# optimism by class
opt_class <- summarySE(data_wide, measurevar="LOT_Opt_T0", groupvars="class", conf.interval=.95) 
```

## self-care
```{r self-care across latent classes}
# boxplot by class
scrBoxPlot <- ggplot(data_wide, aes(class, HamSCQ_T0, fill=class)) + 
  geom_violin() +  
  geom_boxplot(width=0.2, position=position_dodge(1)) +
...
scrOutlierIDs <- data_wide %>%
  identify_outliers(HamSCQ_T0)

# to be conservative and not exclude too much data, we will only consider extreme outliers
scrOutlierIDs$Code[scrOutlierIDs$is.extreme==T] # no extreme outliers

# check assumptions for one-way ANOVA
scrModel  <- lm(HamSCQ_T0 ~ class, data = data_wide)
...
kruskal.test(HamSCQ_T0 ~ class, data = data_wide) # sig. differences across groups

# self-care by class
scr_class <- summarySE(data_wide, measurevar="HamSCQ_T0", groupvars="class", conf.interval=.95) 

data_wide %>% kruskal_effsize(HamSCQ_T0 ~ class)
```

## social support
```{r social support across latent classes}
# boxplot by class
socBoxPlot <- ggplot(data_wide, aes(class, FSozU_T0, fill=class)) + 
  geom_violin() +  
  geom_boxplot(width=0.2, position=position_dodge(1)) +
...
# run Kruskal-Wallis test instead of one-way ANOVA
kruskal.test(FSozU_T0 ~ class, data = data_wide) # sig. differences across groups

data_wide %>% kruskal_effsize(FSozU_T0 ~ class)

# social support by class
soc_class <- summarySE(data_wide, measurevar="FSozU_T0", groupvars="class", conf.interval=.95) 
```

## generalized self-efficacy
```{r gen. self-efficacy across latent classes}
# boxplot by class
gseBoxPlot <- ggplot(data_wide, aes(class, GSE_T0, fill=class)) + 
  geom_violin() +  
  geom_boxplot(width=0.2, position=position_dodge(1)) +
...
# one-way ANOVA
gse.fit <- aov_ez("Code","GSE_T0", data_wide, between=c("class"))
nice(gse.fit, es="pes") # p < .001

# self-efficacy by class
gse_class <- summarySE(data_wide, measurevar="GSE_T0", groupvars="class", conf.interval=.95) 
```
...
corr <- data_wide %>%
  select(LOT_Opt_T0, HamSCQ_T0, FSozU_T0, GSE_T0)
...
mlrdata <- data_wide %>%
  select(LOT_Opt_T0, HamSCQ_T0, FSozU_T0, GSE_T0, Age, Gender, Cohort, class)

# factorize some variables
mlrdata$Gender <- as.factor(mlrdata$Gender)
mlrdata$Cohort <- as.factor(mlrdata$Cohort)

# dummy code DV
mlrdata_long <- mlogit.data(mlrdata, choice = "class", shape = "wide")

# mlr model
mlr.fit = mlogit(class ~ 1 | LOT_Opt_T0 + HamSCQ_T0 + FSozU_T0 + GSE_T0 + Age + Gender + Cohort, data = mlrdata_long, reflevel = "low")
summary(mlr.fit)
...
# Stress and mental health during the pandemic

```{r stress during pandemic}
PSS.mP <- mean(data_wide_pandemic$PSS_P) # moderate perceived stress during pandemic (values > 27 would indicate high stress)
PSS.sdP <- sd(data_wide_pandemic$PSS_P)
  
# pandemic-specific stressor load
hist_pandemic_stressor_count <- ggplot(data_wide_pandemic, aes(x=COR_HS_P_Count)) + 
...
COR_HS_P_Count.m <- mean(data_wide_pandemic$COR_HS_P_Count)
COR_HS_P_Count.sd <- sd(data_wide_pandemic$COR_HS_P_Count)

hist_pandemic_stressor_intensity <- ggplot(data_wide_pandemic, aes(x=COR_HS_P_Int)) + 
  geom_histogram(color="black", fill="gray") +
...
COR_HS_P_Int.m <- mean(data_wide_pandemic$COR_HS_P_Int)
COR_HS_P_Int.sd <- sd(data_wide_pandemic$COR_HS_P_Int)
```

## Most frequently encountered corona stressors

```{r corona stressor item analysis}
cor_hs_items <- data_wide_pandemic %>% 
  select(matches('COR_HS_T4')) %>%
  summarise_all(., sum, na.rm=T)

cor_hs_items

# top 3 stressors:
# 9 (loss of social contact)
# 15 (loss of opportunities for recreational activities)
# 29 (media reports)
```

## Changes in stress and mental health from T3-T4 (impact of the pandemic)

```{r changes in stress and mental health from T3 to T4}

data_wide_pandemic <- list(data_wide_pandemic, data_wide[, c(1,15,16,17,18)]) %>% reduce(left_join, by = "Code") # add T3 data (i.e., last time point before pandemic)

# paired t-tests to compare T3 and T4 stress
t.test(data_wide_pandemic$MIMIS_T3_Frequency, data_wide_pandemic$MIMIS_P_freq, paired = T)
mean(data_wide_pandemic$MIMIS_T3_Frequency)
mean(data_wide_pandemic$MIMIS_P_freq)
...
t.test(data_wide_pandemic$WHO_T3, data_wide_pandemic$WHO_P, paired = T)
mean(data_wide_pandemic$WHO_T3)
mean(data_wide_pandemic$WHO_P)
```

Participants do not seem significantly more stressed during the pandemic than they were before - overall moderate levels of perceived stress (*M* = `r round(PSS.mP,2)`, *SD* = `r round(PSS.sdP,2)`). Of 29 pandemic-specific stressors, participants experienced on average `r round(COR_HS_P_Count.m,2)` (*SD* = `r round(COR_HS_P_Count.sd,2)`) which they rated on a scale from 1 = "not at all burdensome" to 5 = "extremely burdensome". On average stressor intensity was reported as "a bit burdensome" (*M* = `r round(COR_HS_P_Int.m,2)`, *SD* = `r round(COR_HS_P_Int.sd,2)`). Most participants experienced media reports (`r round((cor_hs_items$COR_HS_T4_29/117*100),2)`%), loss of opportunities for recreational activities (`r round((cor_hs_items$COR_HS_T4_15/117*100),2)`%) and loss of social contact (`r round((cor_hs_items$COR_HS_T4_9/117*100),2)`%)
...
# boxplot by class
pssPBoxPlot <- ggplot(data_wide_pandemic, aes(class, PSS_P, fill=class)) + 
  geom_violin() +  
  geom_boxplot(width=0.2, position=position_dodge(1)) +
  labs(y="PSS", x="Class") +
  scale_fill_manual(values=c(colour1, colour2, colour3, colour4)) +
...
pssPOutlierIDs <- data_wide_pandemic %>%
  identify_outliers(PSS_P) # 1

# output extreme outliers
pssPOutlierIDs <- pssPOutlierIDs[pssPOutlierIDs$is.extreme==T,] # none

# check assumptions for one-way ANOVA
pssPModel  <- lm(PSS_P ~ class, data = data_wide_pandemic)
...
# one-way ANOVA
pssP.fit <- aov_ez("Code","PSS_P", data_wide_pandemic, between=c("class")) # p < .01
...
pss_p_class <- summarySE(data_wide_pandemic, measurevar="PSS_P", groupvars="class", conf.interval=.95, na.rm=T) 
```

## Optimism during the pandemic
```{r LOT optimism score during pandemic across latent classes}
# boxplot by class
optPBoxPlot <- ggplot(data_wide_pandemic, aes(class, LOT_Opt_P, fill=class)) + 
  geom_violin() +  
  geom_boxplot(width=0.2, position=position_dodge(1)) +
  labs(y="Optimism", x="Class") +
  scale_fill_manual(values=c(colour1, colour2, colour3, colour4)) +
...
optPOutlierIDs <- data_wide_pandemic %>%
  identify_outliers(LOT_Opt_P) # none

# check assumptions for one-way ANOVA
optPModel  <- lm(LOT_Opt_P ~ class, data = data_wide_pandemic)
...
# one-way ANOVA
optP.fit <- aov_ez("Code","LOT_Opt_P", data_wide_pandemic, between=c("class")) # p < .001
...
opt_p_class <- summarySE(data_wide_pandemic, measurevar="LOT_Opt_P", groupvars="class", conf.interval=.95, na.rm=T) 
```

## Self-Care during the pandemic
```{r HamSCQ score during pandemic across latent classes}
# boxplot by class
scrPBoxPlot <- ggplot(data_wide_pandemic, aes(class, HamSCQ_P, fill=class)) + 
  geom_violin() +  
  geom_boxplot(width=0.2, position=position_dodge(1)) +
  labs(y="Self-Care", x="Class") +
  scale_fill_manual(values=c(colour1, colour2, colour3, colour4)) +
...
scrPOutlierIDs <- data_wide_pandemic %>%
  identify_outliers(HamSCQ_P) # none

# check assumptions for one-way ANOVA
scrPModel  <- lm(HamSCQ_P ~ class, data = data_wide_pandemic)
...
# use Kruskal-Wallis test instead of ANOVA
kruskal.test(HamSCQ_P ~ class, data_wide_pandemic) # p < .001

# Self-Care during pandemic by class
scr_p_class <- summarySE(data_wide_pandemic, measurevar="HamSCQ_P", groupvars="class", conf.interval=.95, na.rm=T) 
```

FSozU was used at T0, it also has component assessing emotional support so should be correlated with this BSSS scale
...
# use Kruskal-Wallis test instead of ANOVA
kruskal.test(BSSS_P_perEmoSupport ~ class, data_wide_pandemic) # p < .05
...
# boxplot by class
gsePBoxPlot <- ggplot(data_wide_pandemic, aes(class, GSE_P, fill=class)) + 
  geom_violin() +  
  geom_boxplot(width=0.2, position=position_dodge(1)) +
...
gsePOutlierIDs <- data_wide_pandemic %>%
  identify_outliers(GSE_P) # 6

# check if extreme outlier
gsePOutlierIDs$Code[gsePOutlierIDs$is.extreme==T] # no extreme outlier

# check assumptions for one-way ANOVA
gsePModel  <- lm(GSE_P ~ class, data = data_wide_pandemic)
...
# use Kruskal-Wallis test instead of ANOVA
kruskal.test(GSE_P ~ class, data_wide_pandemic) # p < .001

# GSE score during the pandemic by class
gseP_class <- summarySE(data_wide_pandemic, measurevar="GSE_P", groupvars="class", conf.interval=.95, na.rm=T) 
```
...
# use Kruskal-Wallis test instead of ANOVA
kruskal.test(RF_P ~ class, data_wide_pandemic) # p < .001

# Dunn test for post-hoc comparisons
pwc <- data_wide_pandemic %>% 
  dunn_test(RF_P ~ class, p.adjust.method = "holm") 
pwc

# get effect size
data_wide_pandemic %>% kruskal_effsize(RF_P ~ class)
...
```{r correlations among predictor variables}
corrP <- data_wide_pandemic %>%
  select(LOT_Opt_P, HamSCQ_P, BSSS_P_perEmoSupport, GSE_P, PSS_P)

c <- rcorr(as.matrix(corrP)) # create correlation matrix
p.mat <- cor.mtest(corrP)
# plot matrix
corrplot(c$r, # strength and direction of correlation
         method = "circle", # shape
         number.cex = .7, # size of shape
...
data_wide_pandemic <- data_wide_pandemic %>%
    mutate(LOT_Opt_P.c = center_this(LOT_Opt_P),
           HamSCQ_P.c = center_this(HamSCQ_P),
           BSSS_P_perEmoSupport.c = center_this(BSSS_P_perEmoSupport),
           GSE_P.c = center_this(GSE_P),
           PSS_P.c = center_this(PSS_P),
...
pairs(~RF_P + LOT_Opt_P.c + HamSCQ_P.c + BSSS_P_perEmoSupport.c + GSE_P.c + PSS_P.c, data = data_wide_pandemic, lower.panel = panel.smooth,
...
# model
pandemic_model <- lm(RF_P ~ class + LOT_Opt_P.c + HamSCQ_P.c + BSSS_P_perEmoSupport.c + GSE_P.c + PSS_P.c, data_wide_pandemic)
summary(pandemic_model)
confint(pandemic_model) # get confidence intervals
lm.beta(pandemic_model) # get standardized betas

# model with covariates
pandemic_model.cov <- update(pandemic_model, .~. + Age.c + Gender + Cohort)
...
pandemic_model.int <- update(pandemic_model, .~. + class:HamSCQ_P.c)
...
sum(data_wide_pandemic$large.residual) # 6 cases --> 4.3%
...
data_wide_pandemic[data_wide_pandemic$large.residual, c("Code", "cooks.distance", "leverage", "covariance.ratios") ] 
# 2 cases with rather large leverage value

# average leverage = (6 + 1)/117 = 0.06
# covariance boundaries: 
# 1 + 3(6+1)/117 = 1.18; 1-3(6+1)/117 = -0.82 # all cases fall within bourndaries
